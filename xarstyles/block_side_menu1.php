...
function sniffer()
{
    ...
    if (preg_match('/MSIE ([0-9].[0-9]{1,2})/', $http_agent, $ver)) {
        $version = $ver[1];
        $agent = 'MSIE';
    } elseif (preg_match('/Opera[ \/]([0-9].[0-9]{1,2})/', $http_agent, $ver)) {
        $version = $ver[1];
        $agent = 'Opera';
    } elseif (preg_match('/Konqueror/([0-9].[0-9]{1,2}/)', $http_agent, $ver)) {
        $version = $ver[1];
        $agent = 'Konqueror';
    } elseif (preg_match('/Netscape6/([0-9].[0-9]{1,2})/', $http_agent, $ver)) {
        $version = '6';
        $agent = 'Netscape';
    } elseif (preg_match('/Netscape\/([0-9].[0-9]{1,2})/', $http_agent, $ver)) {
        $version = $ver[1];
        $agent = 'Netscape';
    } elseif (preg_match('/Firefox\/([0-9]{1,2})/', $http_agent, $ver)) {
        $agent = 'Mozilla Firefox';
        $version = $ver[1];
    } elseif (preg_match('/Mozilla\/([0-9].[0-9]{1,2})/', $http_agent, $ver)) {
        $version = $ver[1];
        $agent = 'Mozilla';
    } elseif (preg_match('/Safari\/([0-9]{1,2})/', $http_agent, $ver)) {
        $agent = 'Safari';
        $version = $ver[1];
    } else {
        $version = 0;
