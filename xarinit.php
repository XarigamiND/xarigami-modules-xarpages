...
function xarpages_upgrade($oldversion)
{
    ...
    switch ($oldversion) {
       ...
       case '0.2.8':
            // Upgrade to 0.2.9 - block instances
            $query = "SELECT DISTINCT instances.xar_id, instances.xar_name FROM $xartable[block_instances] as instances LEFT JOIN $xartable[block_types] as btypes ON btypes.xar_id = instances.xar_type_id WHERE xar_module = 'xarpages'";
            $instances = array(
                        array('header' => 'XarPages Block Title:',
                                'query' => $query,
                                'limit' => 20
                            )
                    );
            xarDefineInstance('xarpages','Block',$instances);
        case '0.3.0':
            //various small fixes and requirement for core 1.4.0
        case '0.3.1':
        case '0.3.2':
        case '0.3.4':
        case '0.3.5': //current version - reflect treeTable upgrade to v3.0 and jquery 1.9.1
        break;
    }
...
function xarpages_delete()
{
    ...
    $objectlist = xarModAPIFunc('dynamicdata','user','getobjects',array('moduleid'=>xarModGetIDFromName('xarpages')));

    foreach ($objectlist as $objectid=>$objectinfo)
    {
        if (!empty($objectid)) {
            $delobject = xarModAPIFunc('dynamicdata','admin','deleteobject',array('objectid' => $objectid,'moduleid'=>xarModGetIDFromName('xarpages')));
        }
       xarLogMessage('DELETING OBJECT :'.$delobject);
    }
