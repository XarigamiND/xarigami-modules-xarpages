...
function xarpages_admin_updatepage($args)
{
    ...
    if (!xarVarFetch('modifypage', 'str', $modifypage, '', XARVAR_NOT_REQUIRED)) return;
     if (!xarVarFetch('modifyreturn', 'str', $modifyreturn, '', XARVAR_NOT_REQUIRED)) return;
    if (!xarVarFetch('previewpage', 'str', $previewpage, '', XARVAR_NOT_REQUIRED)) return;
    ...
    if ($creating) {
        ...
        if ($batch) {
            ...
            xarResponseRedirect(
                xarModUrl('xarpages', 'admin', 'modifypage',
                    array(
                        'batch' => 1,
                        'creating' => 1,
                        'ptid' => $ptid,
            ...
            $msg = xarML('New page was successfully created.');
        ...
        if (empty($previewpage)) {
            $msg = xarML('Page was successfully updated.');
            xarTplSetMessage($msg,'status');
        }
        if (!empty($modifyreturn)) {
            if (!empty($return_url)) {
            ...
            xarResponseRedirect(xarModURL('xarpages', 'admin', 'modifypage', array('pid' => $pid,'preview'=>$previewpage)));
        }
    }
