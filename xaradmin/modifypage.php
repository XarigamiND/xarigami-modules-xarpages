...
function xarpages_admin_modifypage($args)
{
    ...
    if (!empty($pid)) {
        ...
        $data['pid'] = $pid;
        $data['page'] = xarModAPIFunc('xarpages', 'user', 'getpage', array('pid' => $pid));
        ...
        if (!xarSecurityCheck('EditXarpagesPage', 1, 'Page', $data['page']['name'] . ':' . $data['page']['pagetype']['name'])) {
            $msg = xarML('You have no permission to edit this page #(1) type #(2)', $data['page']['name'], $data['page']['pagetype']['name']);
        ...
        $object =  xarModAPIFunc('dynamicdata','user','getobject',
                    array('moduleid' =>xarModGetIdFromName('xarpages'),
                          'itemtype'=> $data['ptid']));
        ...
        if (!xarSecurityCheck('AddXarpagesPage', 1, 'Page', 'All')) {
            $msg = xarML('You have no permission to add a page');
            return xarResponseForbidden($msg);
        }
        ...
        if (!xarVarFetch('name', 'str', $name, '', XARVAR_DONT_SET)) return;
    ...
    $themes = xarModAPIfunc('themes', 'admin', 'getthemelist', array('state' => XARTHEME_STATE_ACTIVE));
    ...
    foreach($themes as $theme) {
        ...
        $templates = xarModAPIfunc(
            ...
            array(
                ...
                'basedir' => $themedir.'/' . $theme['osdirectory'] . '/modules/' . $modinfo['directory'],
                'levels' => 1,
